...
def train_model(rank, world_size, gpu_ids, num_workers, batch_size, learning_rate, num_epochs, tracking=False):
    ...
    # _, imagenet_ds_small = split_train_valid(imagenet_ds, 0.005)
    # print('split')
    ...
    # _, atom_ds = split_train_valid(atom_ds, 0.01)
    ...
    trainer = DDPTrainer(
        ...
        rank=0,
        world_size=1,
        use_data_parallel=True  # Set this to True to use DataParallel
    )
...
def main():
    mp.spawn(
        train_model,
        args=(WORLD_SIZE, GPU_IDS, NUM_WORKERS, BATCH_SIZE, LEARNING_RATE, NUM_EPOCHS),
        nprocs=WORLD_SIZE,
        join=True
    )
