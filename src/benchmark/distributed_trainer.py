...
#     trainer = DistributedTrainer(model=model, loss_func=loss_func, optimizer=optimizer, metrics=[accuracy], scheduler=None,
#                                  device=rank, rank=rank, world_size=world_size, save_every=1, model_path=f'../../../saved_models/{NAME}/')
#     trainer.train(train_dl, epochs, valid_dl=valid_dl, cv_dl=cv_dl, tracking=True)

#     torch.distributed.destroy_process_group()
    
# def run_distributed_training(rank, world_size, trainer, dataloder_list, epochs, tracking=False):
#     os.environ['MASTER_ADDR'] = 'localhost'
#     os.environ['MASTER_PORT'] = '12355'
...
#     train_dl, valid_dl, cv_dl = dataloder_list
#     trainer.train(train_dl, epochs, valid_dl=valid_dl, cv_dl=cv_dl, tracking=tracking)
...
#         model_path='./'
#     )
#     dataloder_list = [train_dl, None, None]
#     epochs = 10
#     run_distributed_training(rank, world_size, trainer, dataloder_list, epochs, tracking=False)
...
def main(rank, world_size):
    ...
    trainer.train(train_dl, epochs=10)
...
if __name__ == "__main__":
    ...
    # download first to prevent download in each process
